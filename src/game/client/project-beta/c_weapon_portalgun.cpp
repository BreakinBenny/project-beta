...
ConVar cl_portalgun_effects_min_alpha("cl_portalgun_effects_min_alpha", "128", FCVAR_CLIENTDLL);
ConVar cl_portalgun_effects_max_alpha("cl_portalgun_effects_max_alpha", "255", FCVAR_CLIENTDLL);
...
CLIENTEFFECT_MATERIAL(PORTALGUN_PORTAL2_FIRED_LAST_GLOW_OLD)
...
BEGIN_NETWORK_TABLE( C_WeaponPortalgun, DT_WeaponPortalgun )
	...
	RecvPropBool(RECVINFO(m_bOlderGun)),
END_NETWORK_TABLE()
...
void C_WeaponPortalgun::StartEffects( void )
{
	...
	if ( m_Parameters[PORTALGUN_PORTAL2LIGHT].GetMaterial() == NULL )
	{
		...
		if (IsOlderGun())
		{
			if (m_Parameters[PORTALGUN_PORTAL2LIGHT].SetMaterial(PORTALGUN_PORTAL2_FIRED_LAST_GLOW_OLD) == false)
			{
				// This means the texture was not found
				Assert(0);
			}
	...
	if ( m_Parameters[PORTALGUN_PORTAL2LIGHT_WORLD].GetMaterial() == NULL )
	{
		...
		if (IsOlderGun())
		{
			if (m_Parameters[PORTALGUN_PORTAL2LIGHT_WORLD].SetMaterial(PORTALGUN_PORTAL2_FIRED_LAST_GLOW_OLD) == false)
			{
				// This means the texture was not found
				Assert(0);
			}
...
void C_WeaponPortalgun::DoEffectIdle( void )
{
	...
	// HACK!! Don't scale all the way down! -reep
	if (m_bPulseUp)
	{
		m_fPulse += gpGlobals->frametime / 2;
		if (m_fPulse > 1.0f)
		{
			m_fPulse = 1.0f;
	...
	m_Parameters[i].GetScale().SetAbsolute(cl_portalgun_effects_max_size.GetFloat() + (cl_portalgun_effects_max_size.GetFloat()) * m_fPulse);
