...
const char *g_pszPlayerModel = "models/player/male_portal_player.mdl";
...
void CPortal_Player::SetupBones( matrix3x4_t *pBoneToWorld, int boneMask )
{
	...
	Studio_BuildMatrices( 
		...
		boneMask );
#else
	/*We need one more arg for Studio_BuildMatrices() -reep*/
	Studio_BuildMatrices(
		pStudioHdr,
		m_PlayerAnimState->GetRenderAngles(),
		adjOrigin,
...
#ifdef SOURCESDK_2013
/*I dunno what this suppressing crosshair thing is but i heard a lot. -reep*/
void CPortal_Player::SuppressCrosshair(bool bSuppress)
{
	bSuppress = m_bSuppressCrosshair;
}
#endif
