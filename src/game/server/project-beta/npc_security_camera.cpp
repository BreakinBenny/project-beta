...
#define	SECURITY_CAMERA_MODEL_OLD		"models/props/security_camera_old.mdl"
...
CNPC_SecurityCamera::CNPC_SecurityCamera( void )
{
	...
	if (PortalGameRules())
	{
		m_boldcam = PortalGameRules()->EarlyInYear();
	}
}
...
void CNPC_SecurityCamera::Precache( void )
{
	...
	PrecacheModel(SECURITY_CAMERA_MODEL_OLD);
...
{ 
	...
	if (m_boldcam)
		SetModel(SECURITY_CAMERA_MODEL_OLD);
	else
